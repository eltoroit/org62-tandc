...
exports.callPublicApex = function(reqHTTP, resHTTP, inputData, callback) {
	...
	var strJSON = JSON.stringify(inputData);
	if (process.env.VERBOSE === true) {
		printLongLines("Data to sent to Destination Org (T&C Org) [JSON]: ", strJSON);
	}
	inputData = Buffer.from(strJSON).toString('base64');
	if (process.env.VERBOSE === true) {
		printLongLines("Data to sent to Destination Org (T&C Org) [BASE64]: ", inputData);
	}
